...
export default function TypeAbonnementForm() {
  ...
  const determineDuree = (type) => {
    switch (type) {
      case 'Abonnement mensuel': return 1;
      case 'Abonnement trimestriel': return 3;
      case 'Abonnement semestriel': return 6;
      case 'Abonnement annuel': return 12;
      default: return '';
    }
  };
  ...
  const handleTypeChange = (e) => {
    ...
    setDuree(determineDuree(selectedType));
  };

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!type || !prix || isNaN(prix)) {
      alert("Veuillez remplir correctement le formulaire. Le prix doit être un nombre.");
      return;
    }

    try {
      if (editId) {
        await axios.put(`/api/type-abonnements/${editId}`, { type, prix, duree_mois: duree });
      } else {
        await axios.post('/api/type-abonnements', { type, prix, duree_mois: duree });
      }
      setType('');
      setPrix('');
      setDuree('');
      ...
      {showModal && (
        <div className="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50">
          <div className="bg-white p-6 rounded-lg shadow-lg max-w-sm w-full">
            ...
            <p className="text-gray-600 mb-4">Êtes-vous sûr de vouloir supprimer cet abonnement ?</p>
